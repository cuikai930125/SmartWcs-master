...
using SMART.WCS.UI.Optimization.DataMembers.OPT0106;
...
{
    /// <summary>
    /// 최적화 수행 처리 - 셀 최적화 수행 관리
    /// </summary>
    public partial class O1006_QPS : UserControl
    {
        #region ▩ Detegate 선언
        ...
        /// List 변수 (선 그래프 객체)
        /// </summary>
        private List<LineSeries2D> g_li2D;
        ...
        public O1006_QPS(List<string> _liMenuNavigation)
        {
            try
            {
                InitializeComponent();
        ...
        #region >> 셀 최적화 수행관리 리스트
        /// <summary>
        /// 셀 최적화 수행관리 리스트
        /// </summary>
        public static readonly DependencyProperty CellProsMgtListProperty
            = DependencyProperty.Register("CellProsMgtList", typeof(ObservableCollection<CellOptProcMgtList>), typeof(O1006_QPS)
                , new PropertyMetadata(new ObservableCollection<CellOptProcMgtList>()));

        /// <summary>
        /// 셀 최적화 수행관리 리스트
        /// </summary>
        public ObservableCollection<CellOptProcMgtList> CellSimuResltLeftGridList
        {
            get { return (ObservableCollection<CellOptProcMgtList>)GetValue(CellProsMgtListProperty); }
            set { SetValue(CellProsMgtListProperty, value); }
        }
        #endregion

        #region >> 셀 최적화 수행관리 결과
        /// <summary>
        /// 셀 최적화 수행관리 결과
        /// </summary>
        public static readonly DependencyProperty CellProsMgtRsltProperty
            = DependencyProperty.Register("CellProsMgtRsltList", typeof(ObservableCollection<CellOptProcMgtRslt>), typeof(O1006_QPS)
                , new PropertyMetadata(new ObservableCollection<CellOptProcMgtRslt>()));

        /// <summary>
        /// 셀 최적화 수행관리 결과
        /// </summary>
        public ObservableCollection<CellOptProcMgtRslt> CellProsMgtRsltList
        {
            get { return (ObservableCollection<CellOptProcMgtRslt>)GetValue(CellProsMgtRsltProperty); }
            set { SetValue(CellProsMgtRsltProperty, value); }
        }
        #endregion
        ...
        public static readonly DependencyProperty SUP_BTCH_DIV_CNTProperty
            = DependencyProperty.Register("SUP_BTCH_DIV_CNT", typeof(int), typeof(O1006_QPS), new PropertyMetadata(null));
        ...
        public static readonly DependencyProperty MIN_SUP_SKU_UNITProperty
            = DependencyProperty.Register("MIN_SUP_SKU_UNIT", typeof(int), typeof(O1006_QPS), new PropertyMetadata(null));
        ...
        private void InitEvent()
        {
            ...
            // 셀 시뮬레이션 버튼 클릭 이벤트
            this.btnCellOpti.PreviewMouseLeftButtonUp += BtnCellOpti_PreviewMouseLeftButtonUp;
            #endregion
        ...
        #region > 차트 바인딩
        #region >> BindingChart - Optimization 결과 차트
        /// <summary>
        /// Optimization 결과 차트
        /// </summary>
        ...
        private void BindingChart(DataTable _dtChartData)
        {
            try
            {
                ...
                this.chartOptimizationRslt.BeginInit();
                ...
                for (int i = iLegendCount; i < this.g_li2D.Count; i++)
                {
                    ...
                    this.xyDiagramRslt.Series.Add(this.g_li2D[i]);
                }
            }
            catch { throw; }
            finally
            {
                this.chartOptimizationRslt.EndInit();
                this.g_li2D = null;
            }
        }
        #endregion

        #region >> GetGenerateData - 셀 Optimization 셀 리스트 차트 데이터 구성 함수
        /// <summary>
        /// 셀 Optimization 셀 리스트 차트 데이터 구성 함수
        /// </summary>
        ...
        private List<ChartDataPointMember> GetGenerateData(string _strRegend, DataTable _dtValue)
        {
            ...
            {   
                //double dArgument    = Convert.ToDouble(drRow["ADD_SUPP_SKU_CNT"]);      // 누적 SKU개수
                double dArgument    = Convert.ToDouble(drRow["BTCH_SEQ"]);          // 배치 순번
                double dValue       = Convert.ToDouble(drRow["CELL_QTY"]);          // 셀 수
        ...
        #region >> GetgenerateDataCell - 셀 Optimization 마스터 수량 결과 차트 데이터 구성 함수
        /// <summary>
        /// 셀 Optimization 마스터 수량 결과 차트 데이터 구성 함수
        /// </summary>
        /// <param name="_strRegend">범례</param>
        /// <param name="_dtValue">차트 데이터</param>
        /// <returns></returns>
        private List<ChartDataPointMember> GetGenerateDataCell(string _strRegend, DataTable _dtValue)
        {
            ...
            foreach (DataRow drRow in dtFilterData.Rows)
            {
                ...
                dValue = Convert.ToDouble(dtFilterData.Rows[0]["CELL_MST_QTY"]);
        ...
        #region > 데이터 관련
        #region >> GetSP_OPT_CELL_QTY_LIST_INQ - 셀 Optimization 최적화 결과 조회
        /// <summary>
        ///  셀 Optimization 최적화 결과 조회
        /// </summary>
        private async Task<DataSet> GetSP_OPT_CELL_QTY_LIST_INQ()
        {
            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue                          = null;
            var strProcedureName                        = "PK_O1006_QPS.SP_OPT_CELL_QTY_LIST_INQ";
        ...
        #region >> GetSP_OPT_CELL_QTY_RSLT_INQ - 셀 유형별 SUM 정보 조회
        /// <summary>
        /// 셀 유형별 SUM 정보 조회
        /// </summary>
        /// <returns></returns>
        private async Task<DataSet> GetSP_OPT_CELL_QTY_RSLT_INQ()
        {
            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue                          = null;
            var strProcedureName                        = "PK_O1006_QPS.SP_CELL_OPT_RSLT_LIST_INQ";
            Dictionary<string, object> dicInputParam    = new Dictionary<string, object>();
            string[] arrOutputParam                     = { "O_CELL_OPT_RSLT_LIST", "O_RTN_RSLT" };
            ...
            dicInputParam.Add("P_WRK_PLAN_YMD",     strWrkPlanYmd);     // 출고일자
            dicInputParam.Add("P_DATA_SET_ID",      strDataSetID);      // 데이터 그룹
            dicInputParam.Add("P_CELL_OPT_SEQ",     iOptSeq);           // 최적화 차수
            #endregion
        ...
        #region >> GetSP_OPT_CBM_CHK - Cell Optimization Validation 체크
        /// <summary>
        /// Cell Optimization Validation 체크
        /// </summary>
        /// <returns></returns>
        private async Task<int> GetSP_OPT_CBM_CHK()
        {
            int iRtnValue       = 0;

            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue                          = null;
            var strProcedureName                        = "PK_O1006_QPS.SP_OPT_CBM_CHK";
            Dictionary<string, object> dicInputParam    = new Dictionary<string, object>();
            string[] arrOutputParam                     = { "O_CBM_OVER_CNT", "O_RTN_RSLT" };

            var strCenterCD     = this.BaseClass.CenterCD;                                                      // 센터코드
            var strWrkPlanYmd   = this.BaseClass.GetCalendarValue(this.deWrkPlanWmd);                           // 출고일자
            var strDataSetID    = this.BaseClass.ComboBoxSelectedKeyValue(this.cboDataSetID);                   // 데이터 그룹
            var iRocOptSeq      = Convert.ToInt32(this.BaseClass.ComboBoxSelectedKeyValue(this.cboRocOptSeq));  // ROC 최적화 차수
            #endregion
            ...
            dicInputParam.Add("P_WRK_PLAN_YMD",     strWrkPlanYmd);     // 출고일자
            dicInputParam.Add("P_DATA_SET_ID",      strDataSetID);      // 데이터 그룹
            dicInputParam.Add("P_ROC_OPT_SEQ",      iRocOptSeq);        // 최적화 차수
            #endregion
            ...
            #region 결과 카운트 추출 후 값 리턴
            if (dsRtnValue == null) { return iRtnValue; }
            if (dsRtnValue.Tables.Count != 1) { return iRtnValue; }
            if (dsRtnValue.Tables[0].Rows.Count != 1) { return iRtnValue; }

            return Convert.ToInt32(dsRtnValue.Tables[0].Rows[0]["CBM_OVER_CNT"]);
            #endregion
        }
        #endregion

        #region >> SaveSP_CELL_OPT_SEQ_SAVE - Data Set 기준 최적화 결과 생성 - 최적화 모듈에서 ROC 수행 후 결과값 업데이트
        /// <summary>
        /// Data Set 기준 최적화 결과 생성 - 최적화 모듈에서 ROC 수행 후 결과값 업데이트
        /// </summary>
        /// <param name="_da">데이터베이스 엑세스 객체</param>
        /// <param name="_dicInputParam">Input 파라메터</param>
        /// <returns></returns>
        private async Task<DataSet> SaveSP_CELL_OPT_SEQ_SAVE(BaseDataAccess _da, Dictionary<string, object> _dicInputParam)
        {
            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue                          = null;
            var strProcedureName                        = "PK_O1006_QPS.SP_CELL_OPT_SEQ_SAVE";
            Dictionary<string, object> dicInputParam    = new Dictionary<string, object>();
        ...
        private async void BtnSearch_PreviewMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                DataTable dtOptOptionList   = await HelperClass.GetSP_OPT_QPS_MAX_CELL_OPT_INQ("CELL_OPT", strDataSetID);
                ...
                {
                    // 오류가 발생한 경우
                    this.CellProsMgtRsltList.ToObservableCollection(null);
                    // ERR_INQ - 조회 중 오류가 발생했습니다.
                    this.BaseClass.MsgError("ERR_INQ");
                    return;
                }
                ...
                // 정상 처리된 경우
                this.CellSimuResltLeftGridList  = new ObservableCollection<CellOptProcMgtList>();
                this.CellSimuResltLeftGridList.ToObservableCollection(dsOptCellQtyList.Tables[0]);
                this.gridLeft.ItemsSource       = this.CellSimuResltLeftGridList;
                ...
                // 정상 처리된 경우
                this.CellProsMgtRsltList = new ObservableCollection<CellOptProcMgtRslt>();
                this.CellProsMgtRsltList.ToObservableCollection(dsOptCellQtyRslt.Tables[0]);
                // 데이터 바인딩
                this.gridRightBottom.ItemsSource = this.CellProsMgtRsltList;
                #endregion

                #region + 우측 상단 차트
                // Optimization 결과 차트 바인딩
                this.BindingChart(dsOptCellQtyList.Tables[0]);
                #endregion
            }
            catch (Exception err)
            {
        ...
        #region >> 셀 Optimization 버튼 클릭 이벤트
        /// <summary>
        ...
        private async void CboRocOptSeq_SelectedIndexChanged(object sender, RoutedEventArgs e)
        {
            ...
            DataTable dtComboData       = await Utility.HelperClass.GetSP_OPT_QPS_CELL_OPT_LIST_INQ(strWrkPlanYmd, strSelectedDataSetID, "CELL_OPT", iRocOptSeq);
