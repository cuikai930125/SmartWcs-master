...
using SMART.WCS.UI.Processing.DataMembers.R1006_GAN;
using System;
...
{
    /// <summary>
    /// Processing > Smart Gantry > 재적치 작업조회
    /// </summary>
    public partial class R1006_GAN : UserControl
    {
        #region ▩ Detegate 선언
        ...
        public static readonly DependencyProperty WrkListProperty
            = DependencyProperty.Register("WrkList", typeof(ObservableCollection<WrkInfo>), typeof(R1006_GAN)
        ...
        public static readonly DependencyProperty GridRowCountProperty
            = DependencyProperty.Register("GridRowCount", typeof(string), typeof(R1006_GAN), new PropertyMetadata(string.Empty));
        ...
        public R1006_GAN(List<string> _liMenuNavigation)
        {
            try
            {
                InitializeComponent();
        ...
        #region ▩ 이벤트
        #region > Loaded 이벤트
        private void R1006_GAN_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                if (FirstLoad)
        ...
        #region >> GetSP_WRK_SEARCH - 재적치 작업 리스트 조회
        /// <summary>
        /// 재적치 작업 리스트 데이터조회
        /// </summary>
        private DataSet GetSP_WRK_SEARCH()
        {
            ...
            var strProcedureName = "PK_R1006_GAN.SP_WRK_SEARCH";
            ...
            var strFrWrkYmd = this.BaseClass.GetCalendarValue(this.deFrWrkYmd);                                     // 시작 재적치 작업 번호
            var strToWrkYmd = this.BaseClass.GetCalendarValue(this.deToWrkYmd);                                     // 종료 재적치 작업 번호
            var strBtchNo = this.txtBatchNo.Text.Trim();   // 배치번호
            var strGanWrkStatCd = this.BaseClass.ComboBoxSelectedKeyValue(this.cboGanWorkState);     // 재적치 작업 상태
