...
using SMART.WCS.UI.Analysis.DataMembers.A1004_SRT;
...
{
    /// <summary>
    /// A1004_SRT.xaml에 대한 상호 작용 논리
    /// </summary>
    public partial class A1004_SRT : UserControl
    {
        #region ▩ Detegate 선언
        ...
        public A1004_SRT(List<string> _liMenuNavigation)
        {
            try
            {
                InitializeComponent();
        ...
        /// 좌측 테이블 리스트
        /// </summary>
        public static readonly DependencyProperty ProcessingStatusByRegionProperty
            = DependencyProperty.Register("ProcessingStatusByRegionList", typeof(ObservableCollection<ProcessingStatusByRegion>), typeof(A1004_SRT)
                , new PropertyMetadata(new ObservableCollection<ProcessingStatusByRegion>()));

        private ObservableCollection<ProcessingStatusByRegion> ProcessingStatusByRegionList
        {
            get { return (ObservableCollection<ProcessingStatusByRegion>)GetValue(ProcessingStatusByRegionProperty); }
            set { SetValue(ProcessingStatusByRegionProperty, value); }
        }
        #endregion

        #region >> Grid Row 수 - Left Table
        /// <summary>
        /// Grid Row수
        /// </summary>
        public static readonly DependencyProperty LeftGridRowCountProperty
            = DependencyProperty.Register("LeftGridRowCount", typeof(string), typeof(A1004_SRT), new PropertyMetadata(string.Empty));
        ...
        /// 우측 그래프
        /// </summary>
        public static readonly DependencyProperty ProcessingStatusByRegionGraphProperty
            = DependencyProperty.Register("ProcessingStatusByRegionGraphList", typeof(ObservableCollection<ProcessingStatusByRegion>), typeof(A1004_SRT)
                , new PropertyMetadata(new ObservableCollection<ProcessingStatusByRegion>()));

        private ObservableCollection<ProcessingStatusByRegion> ProcessingStatusByRegionGraphList
        {
            get { return (ObservableCollection<ProcessingStatusByRegion>)GetValue(ProcessingStatusByRegionGraphProperty); }
            set { SetValue(ProcessingStatusByRegionGraphProperty, value); }
        }
        #endregion

        #region >> Grid Row 수 - Right Chart
        /// <summary>
        /// Grid Row수
        /// </summary>
        public static readonly DependencyProperty RightGridRowCountProperty
            = DependencyProperty.Register("RightGridRowCount", typeof(string), typeof(A1004_SRT), new PropertyMetadata(string.Empty));
        ...
        private void InitControl()
        {
            ...
            this.chartXContent.Content = this.BaseClass.GetResourceValue("RGN_CD");                     // X축 : 권역 (RGN_CD)
        ...
        private void BindingChart(DataTable _dtChartData)
        {
            try
            {
                foreach (DataRow drRow in _dtChartData.Rows)
                {
                    ...
                    {
                        case "NML": // 정상
                            drRow["FLAG_NM"] = this.BaseClass.GetResourceValue("NML_SRT_QTY");
                            break;
                        case "ERR": // 오류
        ...
        #region >> GetSP_RGN_RSLT_CHRT_INQ - 권역별 처리현황 조회 (Chart)
        /// <summary>
        /// 권역별 처리현황 조회 (Chart)
        /// </summary>
        private DataSet GetSP_RGN_RSLT_CHRT_INQ()
        {
            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue = null;
            var strProcedureName = "PK_A1004_SRT.SP_RGN_RSLT_CHRT_INQ";
            Dictionary<string, object> dicInputParam = new Dictionary<string, object>();
            string[] arrOutputParam = { "O_RGN_RSLT_CHRT", "O_RSLT" };
            ...
            dicInputParam.Add("P_FM_YMD", strFromDt);                   // 기간 - From Date
            dicInputParam.Add("P_FM_HM", strFromTm);                    // 기간 - From Time

            dicInputParam.Add("P_TO_YMD", strToDt);                     // 기간 - To Date
            dicInputParam.Add("P_TO_HM", strToTm);                      // 기간 - To Time
            #endregion
        ...
        #region >> GetSP_RGN_RSLT_LIST_INQ - 권역별 처리현황 조회 (List)
        /// <summary>
        /// 권역별 처리현황 조회 (Chart)
        /// </summary>
        private DataSet GetSP_RGN_RSLT_LIST_INQ()
        {
            #region 파라메터 변수 선언 및 값 할당
            DataSet dsRtnValue = null;
            var strProcedureName = "PK_A1004_SRT.SP_RGN_RSLT_LIST_INQ";
            Dictionary<string, object> dicInputParam = new Dictionary<string, object>();
            string[] arrOutputParam = { "O_RGN_RSLT_LIST", "O_RSLT" };
        ...
        private void BtnSearch_First_PreviewMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                // 시간대별 처리현황 데이터 조회
                DataSet dsRtnValue = this.GetSP_RGN_RSLT_LIST_INQ();
                ...
                {
                    // 정상 처리된 경우
                    this.ProcessingStatusByRegionList = new ObservableCollection<ProcessingStatusByRegion>();
                    // 오라클인 경우 TableName = TB_COM_MENU_MST
                    this.ProcessingStatusByRegionList.ToObservableCollection(dsRtnValue.Tables[0]);
                }
                else
                {
                    // 오류가 발생한 경우
                    this.ProcessingStatusByRegionList.ToObservableCollection(null);
                    this.BaseClass.MsgInfo(strErrMsg);
                }

                // 조회 데이터를 그리드에 바인딩한다.
                this.gridMaster.ItemsSource = this.ProcessingStatusByRegionList;
                ...
                DataSet dsRtnValue_Graph = this.GetSP_RGN_RSLT_CHRT_INQ();   //우측 차트 정보
                ...
                {
                    // 정상 처리된 경우
                    this.ProcessingStatusByRegionGraphList = new ObservableCollection<ProcessingStatusByRegion>();
                    // 오라클인 경우
                    this.ProcessingStatusByRegionGraphList.ToObservableCollection(dsRtnValue_Graph.Tables[0]);
                }
                else
                {
                    // 오류가 발생한 경우
                    this.ProcessingStatusByRegionGraphList.ToObservableCollection(null);
